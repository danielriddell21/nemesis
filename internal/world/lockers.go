...
func placeLockers(l *level.Level, rng *worldgen.RNG, rooms []Room, want int) []Coord {
	if want <= 0 {
		return nil
	}
	lockers := make([]Coord, 0, want)
	minSpread := (l.W + l.H) / 10
	for _, r := range rooms {
		if len(lockers) >= want {
			return lockers
		}
		...
		if c, ok := lockerSite(l, rng, r); ok && spacedFrom(lockers, c, minSpread) {
			l.Set(c.X, c.Y, TileLocker)
			lockers = append(lockers, c)
		}
	}
	return lockers
}
...
func lockerSite(l *level.Level, rng *worldgen.RNG, r Room) (Coord, bool) {
...
func backsOntoWall(l *level.Level, c Coord) bool {
